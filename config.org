...
First, the package writes its project cache directly in the
~user-emacs-directory~, which is annoying, as it's not included in the
~git~ repository.  Of course, the file could be added to the
~.gitignore~ file, but I've already added the
~transient~ sub-directory, so I might as well put the project cache in there:
...
After that, the ~projectile~ mode can be enabled:

#+begin_src emacs-lisp
  (projectile-mode +1)
#+end_src

And, for good measure, add a custom keyboard prefix, ⌘+p.  (I don't
expect to be printing directly from Emacs anyway.  But who knows?)
