...
#+begin_src emacs-lisp
    ...
    (let* (
         ...
         (user-bin-path (expand-file-name "bin" user-home))
         (user-local-bin-path (expand-file-name ".local/bin" user-home))
         (paths
              (cons user-bin-path
  		  (cons user-local-bin-path
    		          (mapcar (lambda (p) (expand-file-name p brew-prefix)) brew-paths)))))
      (dolist (path paths)
...
~magit~ is an Emacs interface to Git.  Installing it also enables
~git-rebase-mode~ and ~git-commit-mode~, which provide syntax
highlighting and keybindings when Git invokes ~emacsclient~ for
interactive rebase or commit message editing.
...
Adds a mode for editing ~ledger~'s (and ~hledger~'s) journal files:
